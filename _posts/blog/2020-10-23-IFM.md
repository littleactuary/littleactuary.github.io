...
\\( R_p = \sum_{i=1}^{n} x_iR_i  \\),with \\(x_i = \frac{value of asset i}{total value of portfolio}, i = 1, ..., n \\) and \\( \sum_{i=1}^{n} x_i = 1 \\).

The mean (expectation) of the portfolio's return is:
\\[ E[R_p] = \sum_{i=1}^{n} x_iE[R_i]  \\]
We can also compute the covariance of each investment/asset's return:
\\[ Cov[R_i, R_j] = E \large[ \large( R_i - E[R_i] \large) \large(R_j - E[R_j] \large) \large]  \\]
